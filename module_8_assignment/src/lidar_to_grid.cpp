...
private:
	...
	void Lidar_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
		...
		for (size_t i = 0; i < msg->ranges.size(); i++) { // Change int to size_t
			...
			if (cell_x >= 0 && static_cast<unsigned int>(cell_x) < map->info.width && 
			    cell_y >= 0 && static_cast<unsigned int>(cell_y) < map->info.height) { // Add static_cast
